...
##########################################################################################
        ...
        self.guideBtns=[]
        guideText=["▶","■","〓"]
        for index in range(0,len(guideText)):
            guideBtn=QtWidgets.QPushButton(self.PageGuide)
            guideBtn.setGeometry(450+index*80,850,60,60)
            font.setPointSize(20)
            guideBtn.setFont(font)
            guideBtn.setText(guideText[index])
            guideBtn.setStyleSheet("border-style: solid ; border-radius: 10px;border-color:black; border-width: 3px;color:black;")
            
            guideBtn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
            self.guideBtns.append(guideBtn)
